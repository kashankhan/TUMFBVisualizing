...
+ (void)mutateCoordinatesOfClashingAnnotations:(NSArray *)annotations {
    ...
    NSDictionary *coordinateValuesToAnnotations = [AnnotationCoordinateUtility groupAnnotationsByLocationValue:annotations];
    ...
    for (NSValue *coordinateValue in coordinateValuesToAnnotations.allKeys) {
        ...
        if (outletsAtLocation.count > 1) {
            ...
            [AnnotationCoordinateUtility repositionAnnotations:outletsAtLocation toAvoidClashAtCoordination:coordinate];
        }
    }
}
...
+ (void)repositionAnnotations:(NSMutableArray *)annotations toAvoidClashAtCoordination:(CLLocationCoordinate2D)coordinate {
    ...
    for (int i = 0; i < annotations.count; i++) {
        ...
        CLLocationCoordinate2D newCoordinate = [AnnotationCoordinateUtility calculateCoordinateFrom:coordinate onBearing:heading atDistance:distance];
